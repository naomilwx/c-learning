...
def bce_loss(y_true, y_pred, label_smoothing=0):
  loss_fn = tf.keras.losses.BinaryCrossentropy(label_smoothing=label_smoothing, reduction=tf.keras.losses.Reduction.NONE)
...
def train_eval(
  ...
  with tf.compat.v2.summary.record_if(
    ...
    tf_env, eval_tf_env, obs_dim = multiworld_env.load(env_id)
    # print('obs_dim', obs_dim, tf_env.observation_spec().shape)
    ...
    while global_step_val < num_iterations:
      print('running iteration', global_step_val)
